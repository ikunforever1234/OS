...
- **时钟中断处理（IRQ_S_TIMER、ticks、sched_class_proc_tick）**  
  - **对应OS原理知识点**：时钟中断、进程抢占、时间片轮转  
  - **关系**：OS原理中时钟中断用于触发调度，实现CPU资源公平分配；实验中通过ticks计数和proc_tick实现时间片消耗  
  - **差异/扩展**：实验中更直观地展示了计数器、打印次数、最终关机等效果，比理论抽象多了可观测操作

- **进程控制块初始化（alloc_proc）**  
  - **对应OS原理知识点**：PCB
  - **关系**：PCB是操作系统管理进程的核心数据结构，保存进程状态、寄存器、页表等信息；实验中扩展了时间片、调度队列指针、stride值等字段  
  - **差异/扩展**：实验中加入了链表节点、斜堆节点等调度相关字段
- **进程创建（do_fork、copy_mm、copy_thread）**  
  - **对应OS原理知识点**：进程管理、fork系统调用  
  - **关系**：OS原理讲fork创建子进程，复制父进程上下文；实验中实现了内核态的PCB分配、页表复制、以及trapframe拷贝  
  - **差异/扩展**：实验强调内存页复制和时间片初始化，比原理更底层更具体一些

- **调度器框架（sched_class、run_queue、schedule、wakeup_proc）**  
  - **对应OS原理知识点**：调度算法与策略、调度器设计  
  - **关系**：OS原理讲解RR、Stride、优先级等算法；实验中将算法抽象为sched_class，实现插件式可切换调度，更换接口即可  
  - **差异/扩展**：实验提供了函数指针接口、链表+斜堆数据结构

#### 2. OS原理中重要但实验未覆盖的知识点


- **多核调度**  ：实验在QEMU单核模拟，不涉及多核调度和CPU负载均衡策略

- **中断优先级/嵌套处理**：实验中仅处理时钟中断和trap流程，未涉及多级中断优先级和中断嵌套问题

- **死锁检测**：实验中进程间同步简单,没有涉及到复杂锁，未涉及死锁分析。

- **I/O子系统调度策略**  ：实验主要CPU调度，未涉及磁盘、网络等I/O设备调度

#### 3. 总结理解

- 实验知识点体现了OS原理中**进程管理与调度**的具体实现，把理论中的PCB、调度算法、时间片管理、fork/exec操作落到代码上，并可观察效果。  

- 通过实验，我们理解了OS原理中的抽象概念（如调度算法策略模式、延迟调度need_resched标志）在实际内核代码中的具体体现和实现细节。
