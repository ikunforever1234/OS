...
它保存了``32``个通用寄存器以及剩余的与中断异常处理相关的寄存器，在进程从用户空间跳转到内核空间时，系统调用会改变寄存器的值。我们可以通过调整中断帧来使的系统调用返回特定的值，比如设置要执行的函数和要传递的参数等。
...
当我们传入的``create``参数为``0``时，我们就只能进行查找操作，而若``create``参数为``1``，我们如果查找不到就可以进行分配，因此查找和分配由``create``这个参数来决定，这样的实现相对简洁，并且逻辑直观，控制也比较方便，因此我们也没有必要拆开，并且这样设计，在有多个进程连续切换时，可以避免额外的函数调用开销。


### 五、总结

在这里，我们总结实验中重要的知识点及其与原理之间的对应关系：

首先，分页机制中，通过 ``get_pte()`` 实现了多级页表项查找和按需分配，这与原理课程中虚拟内存管理的分页策略对应。理论中关注页表结构、地址转换流程以及TLB，而实验更多偏向操作层面的实现，比如通过 ``satp`` 寄存器完成页表根切换等。二者的主要差异在于理论强调机制和策略，如分段、分页、请求调页等，而实验直接实现了 ``SV39`` 分页并采用“按需建表”的方式。

其次，在进程切换， ``switch_to`` 汇编函数保存和恢复上下文寄存器，这与原理中的上下文切换概念一致。理论侧重于调度算法和上下文保存的原理，而实验关注寄存器实际压栈、恢复及内核栈切换的具体代码。``trapframe`` 的实现用于处理中断和异常时保存寄存器状态，这与理论中“陷入内核”阶段对应。

第三，实验中通过 ``local_intr_save / intr_restore`` 完成关中断保护临界区，与理论中“原子性”和“屏蔽中断”对应。理论讲述通过屏蔽中断或锁机制避免并发冲突，实验通过读写 ``sstatus`` 的 ``SIE`` 位来实现关闭和恢复中断级别的控制，体现机制落地。

最后，在页表与进程地址空间的关系里，实验强调每个进程拥有独立的 ``pgdir`` 来实现地址空间切换，与理论中“进程拥有独立虚拟空间”的概念相符。差异在于理论关注逻辑与保护，而实验强调与硬件交互。
