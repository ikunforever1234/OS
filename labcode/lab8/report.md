...
在本实验中，我们实现了VFS层来统一管理SFS文件系统、stdin、stdout、disk0等不同设备，为上层提供统一的open/read/write/close接口。这与OS原理中的虚拟文件系统概念完全对应——VFS是操作系统内核与具体文件系统之间的抽象层，使得系统能够支持多种文件系统共存。实验中的实现进行了简化，仅支持一种具体文件系统（SFS），但完整体现了分层抽象的核心思想。而在实际操作系统中，VFS需要支持多种文件系统的动态挂载和卸载，复杂度更高。

##### 文件系统分层架构

实验实现了完整的四层架构：通用文件访问接口层、文件系统抽象层（VFS）、SFS文件系统层和外设接口层。这种分层设计是OS原理中的经典架构模式，目的是降低各层之间的耦合，使每层只需要与相邻层交互。实验通过函数指针的方式实现了层间接口，完整体现了分层设计的优势。这种架构使得替换具体文件系统实现变得容易，只需要实现对应的接口函数即可。

##### 索引节点（inode）

实验中实现了两种形式的inode：存储在磁盘上的sfs_disk_inode和驻留在内存中的sfs_inode。磁盘inode存储文件元数据和数据块索引信息，采用12个直接索引加1个间接索引的方式；内存inode则增加了dirty标志、semaphore互斥锁等运行时信息。这与OS原理中inode作为文件系统核心数据结构的定位一致，体现了文件名与文件内容分离的设计思想。不过实验中的实现较为简化：每个inode占用完整的4KB block，而实际系统为了节省空间会更加紧凑；另外实验只支持一级间接索引，实际系统通常支持二级甚至三级间接索引以支持更大的文件。

##### 目录项（dentry）

实验中的sfs_disk_entry结构体用于存储文件名和inode编号的映射关系，这是实现目录功能的基础。在OS原理中，dentry不仅用于路径解析，还常被缓存在内存中以加速文件查找。实验的实现相对简单，dentry仅存在于磁盘上，不进行内存缓存，这意味着每次路径解析都需要访问磁盘。而在实际的Linux等系统中，会维护一个dentry cache来显著提升路径查找的性能。

##### 文件描述符机制

实验为每个进程实现了files_struct结构，其中包含fd_array数组。每个打开的文件对应一个file结构，记录了读写位置、访问权限、指向inode的指针等信息。这完整实现了OS原理中文件描述符的概念——文件描述符是进程访问文件的句柄，屏蔽了底层实现细节。实验清晰地展示了fd → file → inode这三级映射关系，这是UNIX系统文件访问机制的核心。

##### 设备抽象

实验将stdin、stdout、disk0等设备都抽象为实现了device接口的对象，通过VFS进行统一管理。这体现了UNIX的"一切皆文件"哲学，即通过统一的文件接口来访问不同类型的资源，大大简化了编程模型。不过实验并未完整区分字符设备与块设备，在实际系统中这两类设备的缓冲和调度策略是不同的。

##### SFS磁盘布局

实验中的Simple File System采用了简单直观的磁盘布局：superblock存储魔数和block总数等全局信息，随后是root inode，接着是freemap位图，最后是数据块区域。这种布局方式体现了OS原理中文件系统需要平衡空间利用率和访问效率的设计考量。不过SFS进行了极度简化，缺少现代文件系统的日志、B树索引等高级特性，freemap使用简单的位图结构，在大规模系统中需要更高效的空闲空间管理结构。

##### 路径解析

实验实现了vfs_lookup函数，通过调用sfs_lookup来逐级解析路径，最终返回目标文件的inode。这个过程对应了OS原理中的目录树遍历操作。不过实验的实现相对简单，未完整支持多级目录解析，也没有处理符号链接、挂载点穿越等复杂场景。实际系统中的路径解析还需要进行权限检查、处理特殊路径（如..和.）等工作。

对比我们现在的文件系统，现代文件系统还包含许多高级特性，如写时复制（COW）、快照、去重、压缩等，这些在Btrfs、ZFS等文件系统中都有实现。实验的SFS仅实现了最基础的功能，与现代文件系统的功能还有很大差距。这些未实现的特性虽然增加了系统复杂度，但对于构建生产环境的文件系统是必不可少的。